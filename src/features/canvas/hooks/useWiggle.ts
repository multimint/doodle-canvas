...
export function useWiggle(
  ...
  const applyFrame = useCallback((fi: number) => {
    registryRef.current.forEach((entry) => {
      if (entry.kind === 'shape') {
        ...
        if (entry.variants)
          setPoints(entry.node as Konva.Line, entry.variants[fi]);
      } else {
    ...
    if (live && live.node.getLayer()) {
      if (live.isShape) live.node.setAttr('animT', fi);
        ...
        setPoints(live.node as Konva.Line, live.variants[fi]);
    }
  }, []);
  ...
  const resetAll = useCallback(() => {
    registryRef.current.forEach((entry) => {
      ...
      else if (entry.kind === 'line' && entry.base)
        (entry.node as Konva.Line).points(entry.base);
  ...
  const registerStroke = useCallback(
    (id: string, node: Konva.Node, stroke: Stroke) => {
  ...
  const setFrozenText = useCallback(
    (ids: string[]) => {
